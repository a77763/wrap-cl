...
namespace clcore{
    ...
    CLContext getCLContext()
    {
        ...
        {
            cl_int error;
            context = clCreateContext( NULL, 1,getCLDevice(), NULL, NULL, &error);
    ...
    CLQueue getCLQueue(){
        ...
        {
            cl_int error;
            queue = clCreateCommandQueue(getCLContext(), *id, 0, &error);
            checkError("Create queue", error); 
        }
        return queue;
    }
    void releaseCLQueue(){
        cl_int error = clReleaseCommandQueue(queue);
        checkError("Release queue", error);
    }
    void releaseCLContext(){
        cl_int error = clReleaseContext(context);
        checkError("Release context",error);
    }
